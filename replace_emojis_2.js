...
const svgIcons = {
  lock: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>`,
  idcard: `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="8" cy="11" r="3"/><line x1="14" y1="10" x2="19" y2="10"/><line x1="14" y1="14" x2="19" y2="14"/></svg>`,
  ...
  cross: `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`,
...
indexHtml = indexHtml.replace(/🔐/g, svgIcons.lock);
fs.writeFileSync('index.html', indexHtml);

let appJs = fs.readFileSync('app-v3.js', 'utf8');
appJs = appJs.replace(/🪪/g, svgIcons.idcard);
appJs = appJs.replace(/⚠/g, svgIcons.alert);
appJs = appJs.replace(/✅/g, svgIcons.check);
appJs = appJs.replace(/✓/g, svgIcons.check);
appJs = appJs.replace(/✗/g, svgIcons.cross);
appJs = appJs.replace(/🎉/g, svgIcons.star);
appJs = appJs.replace(/💰/g, svgIcons.money);
appJs = appJs.replace(/📅/g, svgIcons.calendar);
appJs = appJs.replace(/📋/g, svgIcons.clipboard);
appJs = appJs.replace(/📍/g, svgIcons.mappin);
appJs = appJs.replace(/📞/g, svgIcons.phone);
appJs = appJs.replace(/🔔/g, svgIcons.bell);
appJs = appJs.replace(/🕐/g, svgIcons.clock);
appJs = appJs.replace(/ℹ/g, svgIcons.info);
